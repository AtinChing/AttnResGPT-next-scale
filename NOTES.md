...
Since they were imprecise about exact block counts per sweep model, we use num_blocks=8 for ladder points wherever depth allows. When depth does not divide evenly, we distribute the remainder across blocks so sizes differ by at most 1 (e.g. 12 layers / 8 blocks -> [2,2,2,2,1,1,1,1]), rather than dumping the remainder into a single oversized final block as the paper's wording suggests. Then we state that as a fidelity improvement: "the original used approximately 8 blocks without specifying per-model counts; we fix num_blocks=8 across ladder sizes and balance block sizes for a controlled comparison." That's a defensible, maybe superior, methodological choice, and it's the kind of precision-where-the-original-was-vague that reviewers like in a reproduction (according to Claude).

## 12. Fig 5b 30M reproduction note
"Fig 5b reproduced at 30M (3 seeds): baseline grows to peak ~17 (dilution), Full bounded low (~1.7-5, convex combination), Block intermediate with visible reset/sawtooth at block boundary (~9 down to ~3.7). All three paper behaviors present. Sawtooth coarse at 30M (3 blocks); will be cleaner at ladder sizes with 8 blocks."
